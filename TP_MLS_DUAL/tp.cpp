...
void dualContouring(BasicANNkdTree const &kdtree){
    ...
    // so oui, on rajoute un point au centre de la cellule
    // mtn on se soucie plus que de ces points centrés dans les voxels
    // ensuite, on va aprcourir toutes les aretes de la grille (en x en y et en z)
    // et si les deux extreemtiés de l'arete ont un signe différent 
    // alors on relie les 4 centres qui entourent l'arete
    // en créant deux traingles
    // on orient les normales des triangles vers le coté positif de l'arete
